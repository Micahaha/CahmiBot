    ...
    public void onMessageReceived(MessageReceivedEvent event) {
        ...
        if (args[0].equalsIgnoreCase(Main.prefix + "ban")) {
            ...
            if(args.length < 2 || mentionedMembers.isEmpty())
            {
                ...
                error.setDescription("Usage: " + Main.prefix + "ban [Member]");
            ...
            try
            {
                guild.ban(target, Integer.parseInt(args[2])).complete();
                EmbedBuilder success = new EmbedBuilder();
                success.setColor(Color.GREEN);
                success.setTitle("Member " + target.getEffectiveName() + "has been banned from " + guild.getName() + "for " + args[2]);
            }
            catch(Exception e)
            {
                ...
                EmbedBuilder error = new EmbedBuilder();
                error.setColor(Color.red);
                error.setTitle("You lack the permissions to ban " + target.getEffectiveName());
                error.setDescription("You may only ban members with a role beneath your own!");
                ...
                if(!event.getMember().hasPermission(Permission.ADMINISTRATOR))
                {
                    error.setColor(Color.red);
                    error.setTitle("You lack the permissions to ban " + target.getEffectiveName());
                    error.setDescription("you may only ban members with a role beneath the bot!");
